	...
	case 0x1b: // ^[ Escape
		...
		case '[':
			...
			case 'M':
				...
				event = MouseEvent{
					Pos{int(gy) - 32, int(cx) - 32}, down, button, mod}
				return
			}
		CSISeq:
			for {
...
func ctrlModify(r rune) ui.Key {
	switch r {
	// TODO(gyrux): Are the following special cases universal?
	case 0x0:
		return ui.K('`', ui.Ctrl) // ^@
	case 0x1e:
...
var g3Seq = map[rune]ui.Key{
	...
	// NOTE(gyrux): According to urxvt's manpage, \eO[ABCD] sequences are used for
...
//
// NOTE(gyrux): The list is taken blindly from xterm-keys.c in the tmux source
