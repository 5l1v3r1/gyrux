...
// SetupGlobal sets up the terminal for the entire Gyrux session.
...
func setupVT(out *os.File) error {
	...
	*/
	...
	/*
		...
		characters are (notably emojis and some dingbats) with gyrux. When that
		happens, gyrux becomes wrong about where the cursor is when it writes
		its output, and the effect can be disastrous.

		If we turn off autowrap, the terminal won't insert any newlines behind
		the scene, so gyrux is always right about which line the cursor is.
...
func restoreVT(out *os.File) error {
	...
	// https://github.com/entynetproject/gyrux/pkg/issues/629 for an example.
	s += "\r"
	_, err := out.WriteString(s)
	return err
}
