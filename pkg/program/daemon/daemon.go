...
// (typically the first Gyrux shell session) for spawning a daemon.
type Daemon struct {
	// BinPath is the path to the Gyrux binary itself, used when forking. This
...
func (d *Daemon) Spawn() error {
	...
	if binPath == "" {
		...
		if err != nil {
			return errors.New("cannot find gyrux: " + err.Error())
		}
		binPath = bin
	}
