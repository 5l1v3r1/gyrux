...
import (
	...
	daemonmod "github.com/entynetproject/gyrux/pkg/eval/daemon"
	"github.com/entynetproject/gyrux/pkg/eval/re"
	"github.com/entynetproject/gyrux/pkg/eval/store"
	"github.com/entynetproject/gyrux/pkg/eval/str"
	daemonp "github.com/entynetproject/gyrux/pkg/program/daemon"
)
...
var errInvalidDB = errors.New("daemon reported that database is invalid. If you upgraded Gyrux from a pre-0.10 version, you need to upgrade your database by following instructions in https://github.com/entynetproject/upgrade-db-for-0.10/")
...
func InitRuntime(binpath, sockpath, dbpath string) (*eval.Evaler, string) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "warning: cannot create data directory ~/.gyrux")
	} else {
		if dbpath == "" {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "cannot get runtime dir /tmp/gyrux-$uid, falling back to data dir ~/.gyrux:", err)
		runDir = dataDir
	}
	if sockpath == "" {
	...
	if sockpath != "" && dbpath != "" {
		...
		// TODO(gyrux): Connect to daemon and install daemon module
		// asynchronously.
