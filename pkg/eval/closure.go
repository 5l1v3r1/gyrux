...
// Closure is a closure defined in Gyrux. Each closure has its unique
// identity.
type Closure struct {
	ArgNames []string
...
func (c *Closure) Call(fm *Frame, args []interface{}, opts map[string]interface{}) error {
	...
	// BUG(gyrux): When evaluating closures, async access to global variables
	// and ports can be problematic.

	// Make upvalue namespace and capture variables.
	// TODO(gyrux): Is it safe to simply assign ec.up = c.Captured?
	fm.up = make(Ns)
