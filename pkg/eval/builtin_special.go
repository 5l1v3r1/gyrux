...
// compiler.
//
// Flow control structures are also implemented as special forms in gyrux, with
...
func compileUse(cp *compiler, fn *parse.Form) effectOpBody {
	...
	case 2:
		// TODO(gyrux): Allow using variable as module path
		spec = mustString(cp, fn.Args[0],
			"module spec should be a literal string")
...
func loadModule(fm *Frame, spec string) (Ns, error) {
	if strings.HasPrefix(spec, "./") || strings.HasPrefix(spec, "../") {
		...
		path := filepath.Clean(dir + "/" + spec + ".gyl")
	...
	if code, ok := fm.bundled[spec]; ok {
		return evalModule(fm, spec, NewInternalGyruxSource(false, spec, code))
	}
	if fm.libDir == "" {
		return nil, noSuchModule{spec}
	}
	return loadModuleFile(fm, spec, fm.libDir+"/"+spec+".gyl")
}
