...
// Conversion between native and Gyrux values.
//
// Gyrux uses native Go types most of the time - string, bool, hashmap.Map,
...
// code they are distinct types, but Gyrux uses string for all numbers.
// Similarly, Gyrux uses string to represent runes. In all cases, there is a
// many-to-one relationship between Go types and Gyrux types.
//
// Conversion from Go value to Gyrux value can happen without knowing the
...
// Go value corresponding to the Gyrux value "1" can be string("1"), int(1),
...
// ScanToGo converts an Gyrux value to a Go value. the pointer points to. It
...
// Scanner is implemented by types that can scan an Gyrux value into itself.
type Scanner interface {
	ScanGyrux(interface{}) error
}

// FromGo converts a Go value to an Gyrux value. Conversion happens when the
...
func gyToFloat(arg interface{}) (float64, error) {
	switch arg := arg.(type) {
	case float64:
		return arg, nil
	case string:
...
func gyToInt(arg interface{}) (int, error) {
	switch arg := arg.(type) {
	case float64:
		i := int(arg)
		if float64(i) != arg {
...
func gyToRune(arg interface{}) (rune, error) {
	ss, ok := arg.(string)
	if !ok {
